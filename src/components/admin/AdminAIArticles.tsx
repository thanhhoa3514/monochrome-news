...
const AdminAIArticles = () => {
    ...
    const handleGenerate = () => {
        ...
        // Simulate AI Process with Terminal Logs
        setTimeout(() => addLog(`Connecting to Gemini AI...`), 500);
        setTimeout(() => addLog(`Authenticating secure session...`), 1500);
        setTimeout(() => addLog(`Analyzing topic '${aiCategory}' with ${tone} tone...`), 2500);
        setTimeout(() => addLog(`Brainstorming article concepts...`), 4000);

        // Simulate drafting each article
        for (let i = 1; i <= aiArticleCount; i++) {
            setTimeout(() => addLog(`Drafting article ${i}/${aiArticleCount}...`), 4500 + (i * 1500));
        }

        if (includeImage) {
            setTimeout(() => addLog(`Generating image keywords...`), 5000 + (aiArticleCount * 1500));
        }

        setTimeout(() => {
            addLog(`Formatting HTML content...`);
            addLog(`Finalizing output...`);
        }, 6000 + (aiArticleCount * 1500));

        // Finish generation
        setTimeout(() => {
            const newArticles = Array.from({ length: aiArticleCount }).map((_, i) => ({
                id: Date.now() + i,
                title: `Generated Article ${i + 1} about ${aiCategory}`,
                category: aiCategory,
                summary: `This is a ${tone} summary for article ${i + 1} in ${language}. It is approximately ${length} length.`,
                content: `<p>This is the full content generated by AI.</p><h2>Key Insights</h2><ul><li>Point 1</li><li>Point 2</li></ul><p>Conclusion text here.</p>`,
                image_keyword: `${aiCategory} news ${i + 1}`,
            ...
            addLog(`Success! Generated ${aiArticleCount} articles.`);
            toast({
                title: "Generation Complete",
                description: `Successfully generated ${aiArticleCount} articles.`,
            });
        }, 7500 + (aiArticleCount * 1500));
    };
