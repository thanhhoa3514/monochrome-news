...
import { updateRoleAction } from '@/actions/permissions';
...
const AdminPermissionsClient = ({ initialRoles: roles, initialPermissionsData: permissions }: AdminPermissionsClientProps) => {
    ...
    useEffect(() => {
        if (roles.length > 0) {
            const initialPermissionsObj: Record<number, number[]> = {};
            roles.forEach(role => {
                initialPermissionsObj[role.id] = role.permissions.map(p => p.id);
            });
            setLocalPermissions(initialPermissionsObj);
        }
    }, [roles]);
    ...
    const handleSave = (roleId: number) => {
        startTransition(async () => {
            const result = await updateRoleAction(roleId, localPermissions[roleId]);
            if (result.success) {
                toast({
                    title: "Success",
...
export default AdminPermissionsClient;
